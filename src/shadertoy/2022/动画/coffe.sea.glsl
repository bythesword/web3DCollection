...
float cloudAo(vec3 p, vec3 n, float h) {
	return sdCloud(p + h * n) / h;
}
...
vec3 cloudLighting(vec3 p, float den) {
	vec3 n = cloudNormal(p), col = vec3(2, 1.6, 1.4) * (1. + flash);
...
vec3 getSceneColor(vec3 ro, vec3 rd) {
	...
	if(hideCloud) {
		for(float i = 0.; i < 20.; i++) den += clamp(-sdCloud(cloudP + rd * maxCloudD * i / 20.) * .2, 0., 1.);
	}
